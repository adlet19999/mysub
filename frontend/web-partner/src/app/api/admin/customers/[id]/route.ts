...
export async function GET(_: Request, { params }: RouteContext) {
  ...
  try {
    const response = await fetch(`${BACKEND_BASE_URL}/api/v1/common/admin/customers/${params.id}/`, {
      headers: { Authorization: `Bearer ${token}` },
      cache: "no-store",
    });
    const payload = await response.json();
    if (!response.ok) {
      const result = NextResponse.json({ message: payload.message || "Не удалось загрузить профиль пользователя" }, { status: response.status });
