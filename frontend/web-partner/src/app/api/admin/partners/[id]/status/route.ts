...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  try {
    const response = await fetch(`${BACKEND_BASE_URL}/api/v1/common/admin/partners/${params.id}/status/`, {
      method: "POST",
    ...
    if (!response.ok) {
      const result = NextResponse.json({ message: payload.message || "Не удалось обновить статус партнёра" }, { status: response.status });
